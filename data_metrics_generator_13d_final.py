...
X_train_datasets_5d = load('../datasets_13d_X_train_unsampled.npy',allow_pickle = True)
y_train_datasets_5d = load('../datasets_13d_y_train_unsampled.npy',allow_pickle = True)


matrix = np.empty((14*len(X_train_datasets_5d),8))

threshold = np.arange(0.2, 0.851, 0.05).tolist() ## 0.2 to 0.85 at an interval of 0.05
counter = 0

for j in range(0,14*len(X_train_datasets_5d),14):

    distance = []
    
    linSep, decBoundComp, N_inter,hyperCentres,idx1,idx2 = extractDecisionBoundaryMeasures(X_train_datasets_5d[counter], y_train_datasets_5d[counter], seed=0)
    ...
    samplesPerGroup, groupsPerClass, sdVar = extractTopologyMeasures(X_train_datasets_5d[counter], y_train_datasets_5d[counter])
    voverlap = volume_overlap(X_train_datasets_5d[counter], y_train_datasets_5d[counter])
...
np.savetxt('Data_metrics_13d_7_features_final.csv', matrix.tolist() ,delimiter=',',fmt='%f')
